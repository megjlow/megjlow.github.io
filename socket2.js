...
function Board(port, options, callback) {
  ...
  this.HIGH = 1;
  this.LOW = 0;
  this.pins = [{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT},
	{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT},
	{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT},{mode:this.MODES.OUTPUT}];
  ...
  this.transport.addEventListener('error', function(e) {
	...
	this.transport.addEventListener("message", function(evt) {
		...
		var r = "received message: " + data;
		for(var i=0; i<data.length; i++) {
			r = r + " " + data[i].toString(16);
		}
		console.log(r);
		...
		for (var i = 0; i < data.length; i++) {
			...
			if (board.currentBuffer.length === 0 && byte === 0) {
				continue;
			} else {
				board.currentBuffer.push(byte);
			
			first = board.currentBuffer[0];
			last = board.currentBuffer[board.currentBuffer.length - 1];

			// [START_SYSEX, ... END_SYSEX]
			if (first === START_SYSEX && last === END_SYSEX) {
				console.log("message sysex")
				handler = SYSEX_RESPONSE[board.currentBuffer[1]];
			...
			else if (first === START_SYSEX && (board.currentBuffer.length > 0)) {
				// we have a new command after an incomplete sysex command
				currByte = data[i];
				if (currByte > 0x7F) {
					board.currentBuffer.length = 0;
					board.currentBuffer.push(currByte);
				}
			} 
			else {
				/* istanbul ignore else */
			...
			if (board.currentBuffer.length === 3 && first !== START_SYSEX) {
				console.log("message not sysex")
				// response bytes under 0xF0 we have a multi byte operation
				response = first < START_SYSEX ? (first & START_SYSEX) : first;

				var x = MIDI_RESPONSE[response];
				/* istanbul ignore else */
				if (MIDI_RESPONSE[response]) {
					...
					// is called first.
					if (board.versionReceived || first === REPORT_VERSION) {
						board.versionReceived = true;
						MIDI_RESPONSE[response](board);
					}
					board.currentBuffer.length = 0;
				} 
				else {
...
Board.prototype.addListener = function(name, callback) {
	this.messageQueue[name] = callback;
}

Board.prototype.emit = function(name, evt) {
	if(this.messageQueue[name]) {
		this.messageQueue[name](evt);
	}
	console.log("board emit " + evt);
}
...
Board.prototype.sendString = function(string) {
  var bytes = new ArrayBuffer(string + "\0", "utf8");
  var data = [];
...
if (typeof MAX_SIGNIFICAND == 'undefined') {
...
function decodeCustomFloat(input) {
  ...
  return result * Math.pow(10, exponent);
	...
	ext.getPinMode = function(pin) {
		if(ext.isConnected()) {
			var retval = ext.board.MODES.UNKOWN;
			if(ext.board.pins[pin].mode == ext.board.MODES.INPUT) {
				retval = "input";
			}
	...
	ext.getDigital = function(pin, callback) {
  	...
  	ext.setPwm = function(pin, value) {
  		if(ext.isConnected() && ext.board.pins[pin].mode == ext.board.MODES.PWM) {
			ext.board.pwmWrite(pin, value == 'enable' ? 1 : 0);
	    }
  	}
